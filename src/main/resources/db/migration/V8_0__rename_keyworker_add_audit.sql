...
alter table keyworker_configuration
    add column if not exists allow_auto_allocation boolean,
    add column if not exists reactivate_on         date;

update keyworker_configuration
set allow_auto_allocation = case when auto_allocation_flag = 'Y' then true else false end,
    reactivate_on         = active_date
where allow_auto_allocation is null
   or (reactivate_on is null and active_date is not null);

alter table keyworker_configuration
    alter allow_auto_allocation set not null;
