...
import uk.gov.justice.digital.hmpps.keyworker.dto.PrisonKeyWorkerAgregatedStats;
...
public interface PrisonKeyWorkerStatisticRepository extends CrudRepository<PrisonKeyWorkerStatistic, Long> {
    ...
    @Query("select new uk.gov.justice.digital.hmpps.keyworker.dto.PrisonKeyWorkerAgregatedStats(s.prisonId, " +
            "min(s.snapshotDate), " +
            "max(s.snapshotDate), " +
            ...
            "group by s.prisonId ")
    List<PrisonKeyWorkerAgregatedStats> getAggregatedData(@Param("prisonId") String prisonId, @Param("fromDate") LocalDate fromDate, @Param("toDate") LocalDate toDate);
}
