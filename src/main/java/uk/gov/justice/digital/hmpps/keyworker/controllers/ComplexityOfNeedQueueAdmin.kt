...
import uk.gov.justice.digital.hmpps.keyworker.services.ComplexityOfNeedAdminService

@RestController
@Validated
@ConditionalOnExpression("{'aws', 'localstack'}.contains('\${complexity-of-need-sqs.provider}')")
@RequestMapping("/complexity-of-need-queue-admin", produces = [MediaType.APPLICATION_JSON_VALUE])
class ComplexityOfNeedQueueAdmin(
  private val queueAdminService: ComplexityOfNeedAdminService
) {
