...
public class RoleMigrationService {
    ...
    public void migrate(String caseload) {
        migrateRoles(caseload, SOURCE_ROLE_CODES, "NWEB", TARGET_ROLE_CODES);
    }
    
    void migrateRoles(String sourceCaseload, Set<String> sourceRoleCodes, String targetCaseload, Set<String> targetRoleCodes) {
        ...
        makeRoleAssignmentChange(staffToMigrate, targetCaseload, targetRoleCodes, roleService::assignRole);
        makeRoleAssignmentChange(staffToMigrate, sourceCaseload, sourceRoleCodes, roleService::removeRole);
    }
    ...
    private Set<Long> findStaffMatchingCaseloadAndRoles(String caseloadId, Set<String> roleCodes) {
        ...
        Optional<Set<Long>> staffIds = staffIdSets.reduce(Sets::intersection);
        return staffIds.orElse(Collections.emptySet());
    }

    private void makeRoleAssignmentChange(Set<Long> staffIds, String caseloadId, Set<String> roleCodes, RoleAssignmentChange change) {
        for (Long staffId : staffIds) {
            for (String roleCode : roleCodes) {
                change.accept(staffId, caseloadId, roleCode);
            }
        }
    }

    @FunctionalInterface
    interface RoleAssignmentChange {
        void accept(long staffId, String caseLoad, String roleCode);
    }
}
